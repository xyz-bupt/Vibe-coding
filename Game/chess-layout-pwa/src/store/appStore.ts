...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  saveProgress: (layoutId: string, progress: Partial<LearningProgress>) => {
    // Validate layout ID to prevent injection
    if (!/^[a-zA-Z0-9_\-]+$/.test(layoutId)) {
      console.error('[Security] Invalid layout ID');
      return;
    }
    ...
    const newProgress = {
      ...
      [layoutId]: {
        layoutId,
        completed: get().progress[layoutId]?.completed ?? false,
        currentMove: get().progress[layoutId]?.currentMove ?? 0,
        score: get().progress[layoutId]?.score ?? 0,
    ...
    try {
      SecureStorage.setItem('chess_learning_progress', newProgress);
    } catch (error) {
      console.error('[Security] Failed to save progress:', error);
    }
  },

  // Load learning progress from secure storage
  loadProgress: () => {
    try {
      const saved = SecureStorage.getItem<Record<string, LearningProgress>>('chess_learning_progress');
      if (saved) {
        set({ progress: saved });
      }
    } catch (error) {
      console.error('[Security] Failed to load progress:', error);
    }
  }
}));

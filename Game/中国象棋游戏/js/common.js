...
// 使用 addEventListener 以便与 enhanced.js 兼容
...
function bindGameControls() {
	// 悔棋
	var regretBtn = com.get("regretBtn");
	if (regretBtn) {
	...
	//返回首页
	var gohomeBtn = com.get("gohomeBtn");
	if (gohomeBtn) {
		gohomeBtn.addEventListener("click", function(e) {
			...
			// 隐藏所有子菜单
			var menuIds = ["menuQj", "menuDy", "menuPvp", "menuManual", "menuCollection"];
			for (var i = 0; i < menuIds.length; i++) {
				var menu = com.get(menuIds[i]);
				if (menu) menu.style.display = "none";
			}
			// 隐藏棋谱导航
			var manualNav = com.get("manualNav");
			if (manualNav) manualNav.style.display = "none";
		});
	}

	//重新开始棋局
	var restartBtn = com.get("restartBtn");
	if (restartBtn) {
		restartBtn.addEventListener("click", function(e) {
			if (confirm("是否确定要重新开始？")){
				if (play && play.depth !== undefined) {
					play.isPlay = true;
...
//设置cookie
com.setCookie = function(name, value, days) {
	var expires = "";
	if (days) {
		var date = new Date();

...
export class SettingsModal {
    ...
    private initializeEventListeners(): void {
        ...
        // 关闭按钮
        const closeButtons = this.modal.querySelectorAll('[data-modal-close], [data-modal-cancel]');
        closeButtons.forEach(btn => {
            btn.addEventListener('click', () => this.hide());
        });

        // 保存按钮
        const saveBtn = this.modal.querySelector('[data-modal-confirm]') as HTMLElement;
    ...
    private populateForm(): void {
        const elements = {
            pomodoroDuration: document.getElementById('pomodoro-duration') as HTMLInputElement,
            shortBreakDuration: document.getElementById('short-break-duration') as HTMLInputElement,
            longBreakDuration: document.getElementById('long-break-duration') as HTMLInputElement,
            longBreakInterval: document.getElementById('long-break-interval') as HTMLInputElement,
            dailyGoal: document.getElementById('daily-goal') as HTMLInputElement,
            enableNotifications: document.getElementById('enable-notifications') as HTMLInputElement,
            enableSound: document.getElementById('enable-sound') as HTMLInputElement,
            autoStartBreaks: document.getElementById('auto-start-breaks') as HTMLInputElement,
            autoStartPomodoros: document.getElementById('auto-start-pomodoros') as HTMLInputElement,
            themeSelect: document.getElementById('theme-select') as HTMLSelectElement
        };

        elements.pomodoroDuration.value = (this.settings.timer.pomodoroDuration / 60).toString();
        elements.shortBreakDuration.value = (this.settings.timer.shortBreakDuration / 60).toString();
        elements.longBreakDuration.value = (this.settings.timer.longBreakDuration / 60).toString();
        elements.longBreakInterval.value = this.settings.timer.longBreakInterval.toString();
        ...
        elements.autoStartBreaks.checked = this.settings.notifications.autoStartBreaks;
        elements.autoStartPomodoros.checked = this.settings.notifications.autoStartPomodoros;
